...
void main() {
  ...
  group('ProductsController Tests', () {
    ...
    testWidgets('Fetch Products', (WidgetTester tester) async {
      ...
      await firestore.collection('users').doc(userId).collection('products').add({
        'name': 'Test Product',
        'category': 1,
      ...
      await productsController.fetchProducts(userId);

      expect(productsController.products.length, greaterThan(0));
      expect(productsController.products[0].product.value?.name, 'Test Product');
    });
    ...
    testWidgets('Add Product', (WidgetTester tester) async {
      ...
      await productsController.addProduct(productController);
    ...
    testWidgets('Remove Product', (WidgetTester tester) async {
      ...
      // Add a sample product to Firestore for testing
      DocumentReference docRef = await firestore.collection('users').doc(userId).collection('products').add({
